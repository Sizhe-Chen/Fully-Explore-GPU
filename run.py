...
def get_file_time(file):
    ...
    return [convert_time(os.path.getctime(file)), convert_time(os.path.getmtime(file))]
    ...
    while 1:
        try:
            for i in gpus:
                ...
                os.system(cmd)
                log_table[log_file_name] = [log_file_name, cmd]
                print(get_time(), '   ', cmd)
                time.sleep(2)
            time.sleep(interval)
...
if __name__ == '__main__':
    ...
    parser.add_argument('start_id', help='the first task id to start, 0 for start from scratch')
